...
public class Facebook {
    ...
    String logoutImpl(Context context) throws MalformedURLException, IOException {
        checkUserSession(PeppermintConstant.JSON_KEY_LOGOUT);
        Bundle b = new Bundle();
    ...
    public void dialog(Context context, String action, Bundle parameters, DialogListener listener) {
        ...
        if (action.equals(LOGIN)) {
            parameters.putString(PeppermintConstant.JSON_KEY_TYPE, "user_agent");
            parameters.putString(Track.CLIENT_ID, this.mAppId);
        } else {
            parameters.putString(ServerProtocol.FALLBACK_DIALOG_PARAM_APP_ID, this.mAppId);
            if (isSessionValid()) {
    ...
    public void setAccessExpiresIn(String expiresInSecsFromNow) {
        ...
        if (expiresInSecsFromNow != null) {
            long expires;
            if (expiresInSecsFromNow.equals(a.d)) {
                expires = 0;
            } else {
