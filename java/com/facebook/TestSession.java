...
public class TestSession extends Session {
    ...
    private static synchronized TestSession createTestSession(Activity activity, List<String> permissions, Mode mode, String sessionUniqueUserTag) {
        ...
        synchronized (TestSession.class) {
            ...
            if (Utility.isNullOrEmpty((Collection) permissions)) {
                permissions = Arrays.asList(new String[]{PeppermintConstant.JSON_KEY_EMAIL, "publish_actions"});
            }
    ...
    private static synchronized void retrieveTestAccountsForAppIfNeeded() {
        synchronized (TestSession.class) {
            if (appTestAccounts == null) {
                ...
                testUserNamesParam.putString("fields", PeppermintConstant.JSON_KEY_NAME);
                new Request(null, i.a, testUserNamesParam, null).setBatchEntryDependsOn("testUsers");
    ...
    private static synchronized TestAccount findTestAccountMatchingIdentifier(String identifier) {
        ...
        synchronized (TestSession.class) {
            ...
            } while (!testAccount.getName().contains(identifier));
        }
        return testAccount;
    }
    ...
    private TestAccount createTestAccountAndFinishAuth() {
        ...
        if (this.mode == Mode.SHARED) {
            parameters.putString(PeppermintConstant.JSON_KEY_NAME, String.format("Shared %s Testuser", new Object[]{getSharedTestAccountIdentifier()}));
        }
        ...
        if (error != null) {
            ...
            if (this.mode == Mode.SHARED) {
                testAccount.setName(parameters.getString(PeppermintConstant.JSON_KEY_NAME));
                storeTestAccount(testAccount);
            }
            finishAuthWithTestAccount(testAccount);
            return testAccount;
    ...
    private void deleteTestAccount(String testAccountId, String appAccessToken) {
        Bundle parameters = new Bundle();
        parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, appAccessToken);
    ...
    private String getPermissionsString() {
        return TextUtils.join(i.b, this.requestedPermissions);
    }
