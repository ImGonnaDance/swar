...
class AuthorizationClient implements Serializable {
    ...
    static class AuthDialogBuilder extends Builder {
        ...
        public WebDialog build() {
            ...
            parameters.putString(Track.CLIENT_ID, getApplicationId());
            ...
            if (result != null) {
                ...
                if (currentPermissions == null || !(permissions == null || currentPermissions.containsAll(permissions))) {
                    ...
                    if (!newPermissions.isEmpty()) {
                        addLoggingExtra(AuthorizationClient.EVENT_EXTRAS_NEW_PERMISSIONS, TextUtils.join(i.b, newPermissions));
                    }
    ...
    class WebViewAuthHandler extends AuthHandler {
        ...
        boolean tryAuthorize(final AuthorizationRequest request) {
            ...
            if (!Utility.isNullOrEmpty(request.getPermissions())) {
                String scope = TextUtils.join(i.b, request.getPermissions());
                parameters.putString(ServerProtocol.DIALOG_PARAM_SCOPE, scope);
                addLoggingExtra(ServerProtocol.DIALOG_PARAM_SCOPE, scope);
            }
            parameters.putString(AuthorizationClient.EVENT_EXTRAS_DEFAULT_AUDIENCE, request.getDefaultAudience().getNativeProtocolAudience());
            ...
            if (Utility.isNullOrEmpty(previousToken) || !previousToken.equals(loadCookieToken())) {
                addLoggingExtra(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, a.d);
            } else {
                parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, previousToken);
                addLoggingExtra(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, a.e);
            }
        ...
        private String loadCookieToken() {
            return AuthorizationClient.this.getStartActivityDelegate().getActivityContext().getSharedPreferences(AuthorizationClient.WEB_VIEW_AUTH_HANDLER_STORE, 0).getString(AuthorizationClient.WEB_VIEW_AUTH_HANDLER_TOKEN_KEY, i.a);
        }
    }
    ...
    boolean checkInternetPermission() {
        ...
        if (checkPermission("android.permission.INTERNET") != 0) {
            complete(Result.createErrorResult(this.pendingRequest, this.context.getString(Resource.R(this.context, "R.string.com_facebook_internet_permission_error_title")), this.context.getString(Resource.R(this.context, "R.string.com_facebook_internet_permission_error_message"))));
            return false;
        }
    ...
    private void addLoggingExtra(String key, String value, boolean accumulate) {
        ...
        if (this.loggingExtras.containsKey(key) && accumulate) {
            value = new StringBuilder(String.valueOf((String) this.loggingExtras.get(key))).append(i.b).append(value).toString();
        }
    ...
    boolean tryCurrentHandler() {
        ...
        if (!this.currentHandler.needsInternetPermission() || checkInternetPermission()) {
            ...
            addLoggingExtra(EVENT_EXTRAS_MISSING_INTERNET_PERMISSION, a.e, false);
        }
        return z;
    }
    ...
    Request createGetProfileIdRequest(String accessToken) {
        Bundle parameters = new Bundle();
        parameters.putString("fields", PeppermintConstant.JSON_KEY_ID);
        parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, accessToken);
    ...
    private void logAuthorizationMethodComplete(String method, String result, String errorMessage, String errorCode, Map<String, String> loggingExtras) {
        Bundle bundle;
        if (this.pendingRequest == null) {
            bundle = newAuthorizationLoggingBundle(i.a);
    ...
    static Bundle newAuthorizationLoggingBundle(String authLoggerId) {
        ...
        bundle.putString(EVENT_PARAM_METHOD, i.a);
        bundle.putString(EVENT_PARAM_LOGIN_RESULT, i.a);
        bundle.putString(EVENT_PARAM_ERROR_MESSAGE, i.a);
        bundle.putString(EVENT_PARAM_ERROR_CODE, i.a);
        bundle.putString(EVENT_PARAM_EXTRAS, i.a);
        return bundle;
    }
    ...
    private static String getE2E() {
        ...
        try {
            e2e.put(MercuryDefine.ACTION_INIT, System.currentTimeMillis());
